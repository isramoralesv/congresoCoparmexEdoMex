...
app.factory("Aut", function(FBURL, $firebaseAuth, $firebaseObject) {
	...
	var Aut = {
		...
		crearPerfil: function(uid, usuario){
			...
			//Función para crear el Nodo 'administradores' en caso de que se haya registrado un nuevo usuario
			ref.onAuth(function(authData) {
			  if (authData) {
			    return ref.child("administradores").child(authData.uid).set({
	...
	aut.$onAuth(function(authData) {
		if(authData) {      
	      ...
	      Aut.usuario.profile = $firebaseObject(ref.child('administradores').child(authData.uid).child('profile'));
